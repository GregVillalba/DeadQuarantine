...

{
...

    {
        ...
        Vector3 posicionEsperada =
            posicion + normal * 0.05f;

        Quaternion rotacionEsperada =
            Quaternion.LookRotation(-normal);

        Assert.That(
            decal.transform.position,
            Is.EqualTo(posicionEsperada)
                .Using(Vector3EqualityComparer.Instance)
        );

        Assert.That(
            decal.transform.rotation,
            Is.EqualTo(rotacionEsperada)
...

    {
        ...
        // Esta 4ta instancia hace que la cola
        // supere maxDecals y descarte al más viejo
...

    {
        ...
        Assert.IsNotNull(
            field,
            $"No se encontró el campo '{name}' en DecalManager"
        );
...

    {
        ...
        Assert.IsNotNull(
            method,
            $"No se encontró el método '{methodName}' en DecalManager"
        );
...

    {
        ...
        return (Queue<GameObject>)field.GetValue(
            manager
        );
